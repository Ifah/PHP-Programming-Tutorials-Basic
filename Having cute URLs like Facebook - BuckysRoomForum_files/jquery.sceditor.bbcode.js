...
;(function ($, window, document) {
	...
	var _templates = {
		...
		image:	'<div><label for="link">{url}</label> <input type="text" id="image" value="http://" /></div>' +
				'<div><input type="button" class="button" value="{insert}" /></div>' +
                '<hr>' +
                '<div class="sceditor-image-uploader-cont"><label for="width">{uploadimage}</label> <span class="inputholder"> <input id="sceditor_image_upload" name="sceditor_image_upload" type="file" /> </span></div>' +
                '<hr>' +
	...
	$.sceditor = function (el, options) {
		...
		initEditor = function () {
			...
			$wysiwygEditor = $('<iframe frameborder="0" id="sceditor-frame"></iframe>');
...
// TODO: Trigger custom paste event to allow filtering (pre and post converstion?)
			...
			rangeHelper.restoreRange();
            //Remove HTML Codes if parent is code
     //       if(rangeHelper.getFirstBlockParent().nodeName.toLowerCase() == 'code')
//            {                

                pasteddata = pasteddata.replace(/(<\/li><br>)/ig, "</li>");
                pasteddata = pasteddata.replace(/(<br[\s//]?>)/ig, "8br8");
                
                pasteddata = pasteddata.replace(/(<[^>]+>)/ig, '');        
                pasteddata = pasteddata.replace(/(8br8)/ig, "<br>");        
...
//            }
		...
		base.closeDropDown = function (focus) {
            ...
            ////in order to prevent closing uploading box when db-clicking the image
            if (typeof window.uploadifySelectingFinished != 'undefined' && window.uploadifySelectingFinished == false) {
                return false; 
            }
...
// TODO: Make configurable
		...
		image: {
			exec: function (caller) {
                ...
                var	editor  = this,
					content = _tmpl('image', {
						url: editor._('URL:'),
						width: editor._('Image Width (optional):'),
						height: editor._('Image Height (optional):'),
                        insert: editor._('Insert'),
						uploadimage: editor._('Upload an Image:'),
                ...
                //Call uploader init function
                window.sceditorInstance = editor;
                window.sceditorUploadContentPanel = content;
                initSceditorImageUploader();
	...
	$.sceditor.defaultOptions = {
		 ...
		 * @type {String}
		 */
		toolbarExclude: 'justify,email,ltr,rtl,link,unlink,indent,outdent,cut,copy,paste,pastetext',
...
// TODO:This should check for CSS white-space, should pass it in the function to reduce css lookups which are SLOW!
	...
	$.sceditor.plugins.bbcode.bbcodes = {
		...
		youtube: {
			...
			html: '<iframe width="560" height="315" src="https://www.youtube.com/embed/{0}?wmode=opaque' +
...
*/
function initSceditorImageUploader() {
    
    //Ajax Upload
    jQuery('#sceditor_image_upload').uploadify({
        'swf' : '/js/uploadify/uploadify.swf',
        'uploader' : '/sceditoruploader.php',
        ...
        'onDialogClose'  : function(queueData) {
            //in order to prevent closing uploading box when db-clicking the image
            window.uploadifySelectingFinished = true;
        },
        'onDialogOpen': function(){
            //in order to prevent closing uploading box when db-clicking the image
            window.uploadifySelectingFinished = false;
        },
        'onUploadSuccess' : function(file, data, response) {
            ...
            if(rsp.success == 0)
            {
                ...
                if (typeof window.sceditorInstance != 'undefined') {
                    var contentWidth = window.sceditorUploadContentPanel.find('#width').val();
                    var contentHeight = window.sceditorUploadContentPanel.find('#height').val();
                    ...
                    var imgHtml = '<img src="' + rsp.file + '"' + contentWidth + contentHeight + '/>';
                    window.sceditorInstance.wysiwygEditorInsertHtml(imgHtml);
                    window.sceditorInstance.closeDropDown(true);
                }
        ...
        'onUploadStart' : function(file) {
            ...
            {
                jQuery('#sceditor_image_upload').uploadify('cancel', '*');
                alert('Invalid file type! Please upload JPG, JPEG, PNG or GIF file.');
                
            }else{
                jQuery('#tradeEditForm .loading-wrapper').show();
